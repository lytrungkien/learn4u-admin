...
const GrammarLessonDetail = () => {
	...
	const getLessonDetail = async () => {
		setLoading(true);
		try {
			const res = await axios.get(`${URL}/api/Lesson/GetLessonContent/0/${lessonInfo?.lessonCode}`, { headers });
			if (res.status === 200) {
	...
	const handleDelete = async () => {
		try {
			const res = await axios.delete(`${URL}/api/Admin/delete-grammar-learning/${word.id}`, { headers });
			if (res.status === 200) {
